...
class ConditionalActorVectorField(nn.Module):
    """Actor vector field network for flow matching.
    ...
    @nn.compact
    def __call__(self, observations, actions, next_observations, times=None, is_encoded=False):
        """Return the vectors at the given states, actions, and times (optional).
        ...
        if not is_encoded and self.encoder is not None:
            observations = self.encoder(observations)
            next_observations = self.encoder(next_observations)
        if times is None:
            inputs = jnp.concatenate([observations, next_observations, actions], axis=-1)
        else:
            inputs = jnp.concatenate([observations, next_observations, actions, times], axis=-1)
...
class NextStateVectorField(nn.Module):
    """Actor vector field network for flow matching.
    ...
    @nn.compact
    def __call__(self, observations, next_observations, times=None, is_encoded=False):
        """Return the vectors at the given states, actions, and times (optional).
        ...
        if times is None:
            ...
            inputs = jnp.concatenate([observations, next_observations, times], axis=-1)
